...
class TaskBuilder : IModule {
    ...
    auto isBlocking(bool isBlocking = true) -> TaskBuilder& {
    ...
    auto timeoutSeconds(float timeoutSeconds) -> TaskBuilder& {
        if (timeoutSeconds > 0) {
    ...
    auto retry(int attempts, float seconds) -> TaskBuilder& { return maxAttempts(attempts).timeoutSeconds(seconds); }
